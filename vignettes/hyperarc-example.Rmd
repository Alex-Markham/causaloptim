---
title: "The hyperarc encoding: direct enumeration"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{The hyperarc encoding: direct enumeration}
...
optimising over all latent distributions compatible with the observed data.
Sachs et al. (2023) reparameterise the causal model via *response functions*,
...
This vignette accompanies Markham et al. (2026) and works through the running
example from that paper: the binary confounded DAG with $V_L = \{X_1\}$,
$V_R = \{X_2, X_3\}$, and an internal edge $X_2 \to X_3$.
We enumerate $H$ directly---in topological order, without ever materialising
$H_\text{full}$---display all 16 candidate hyperarcs (marking the 4 invalid
ones), and verify that both encodings give the same sharp bounds.


# Setup

```{r graph-setup}
g <- graph_from_literal(X1 -+ X2, X2 -+ X3) |> initialize_graph()
V(g)$nvals    <- 2L
V(g)$latent   <- 0L
V(g)$leftside <- 0L
V(g)[name == "X1"]$leftside <- 1L
...
$X_2$ and $X_3$, and a latent parent $U_1$ of $X_1$ (as required by the model
class: every $V_L$ node must have a latent parent).
...
card_R <- prod(sapply(right_vars, function(j) {
  nv <- V(g)[j]$nvals
  pa <- names(neighbors(g, j, mode = "in"))
...
unrealised $X_2$ value is never observed, the two Sachs response functions
that agree on the realised $X_2$ input but disagree on the unrealised one
...
> To identify $H$ they iterate over all
...
`optimize_effect_2` formats those linear forms into a single `MAX {...}` /
`MIN {...}` string, so to compare element-wise we re-run vertex enumeration
on the Sachs LP (`mod$R`, `mod$c0`) using the same `enumerate_vertices`
helper applied to the hyperarc LP above.

```{r compare}
sachs_lower <- enumerate_vertices( mod$R,        mod$c0, mod$parameters)
sachs_upper <- enumerate_vertices(-mod$R, -1.0 * mod$c0, mod$parameters)

sachs_lb <- sort(trimws(sachs_lower))
sachs_ub <- sort(trimws(sachs_upper))
